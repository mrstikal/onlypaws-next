...
describe('DeleteCommentInlineButton', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders delete button', () => {
    render(<DeleteCommentInlineButton commentId="5" />);
  ...
  it('asks for confirmation and skips API call when cancelled', async () => {
    ...
    expect(global.confirm).toHaveBeenCalledWith('Opravdu smazat tento komentář?');
  ...
  it('calls DELETE API and refreshes on success', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('/api/cms/comments/5', { method: 'DELETE' });
  ...
  it('re-enables button after request completes', async () => {
    ...
    render(<DeleteCommentInlineButton commentId="6" />);
    const user = userEvent.setup();

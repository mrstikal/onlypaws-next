...
describe('DeleteLikeInlineButton', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('renders delete button', () => {
    render(<DeleteLikeInlineButton likeId="99" />);
  ...
  it('asks for confirmation and skips API call when cancelled', async () => {
    ...
    expect(global.confirm).toHaveBeenCalledWith('Opravdu smazat tento lajk?');
  ...
  it('calls DELETE API and refreshes on success', async () => {
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith('/api/cms/likes/99', { method: 'DELETE' });
  ...
  it('re-enables button after request completes', async () => {
    ...
    render(<DeleteLikeInlineButton likeId="100" />);
    const user = userEvent.setup();

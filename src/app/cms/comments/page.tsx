...
import { loadCommentsPage, type CmsCommentsSortKey } from '@/lib/server/cms/comments/queries';
...
const VALID_SORT_KEYS: CmsCommentsSortKey[] = ['created_at', 'id', 'user_id', 'likes_count', 'commentable_type', 'commentable_id'];

function normalizeSortKey(value: string | null, defaultKey: CmsCommentsSortKey): CmsCommentsSortKey {
  if (value && VALID_SORT_KEYS.includes(value as CmsCommentsSortKey)) {
    return value as CmsCommentsSortKey;
  }
  return defaultKey;
}
...
function sortKeyLabel(key: string) {
  ...
  if (key === 'likes_count') return 'Lajky';
  if (key === 'commentable_type') return 'Typ cíle';
  if (key === 'commentable_id') return 'Cíl';
  if (key === 'created_at') return 'Vytvořeno';
  return key;
}

export default async function CmsCommentsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const data = await loadCommentsPage({
    scope,
    ...
    type: type === 'post' || type === 'pet' ? type : null,
  });
  ...
  const colCount = 6 + (staff ? 1 : 0) + (staff ? 1 : 0);

  return (
    <main className="p-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-lg font-semibold">Komentáře</h1>
          <p className="mt-1 text-sm text-gray-600">{scope === 'mine' ? 'Moje (jen pro čtení)' : 'Všechny'}</p>
        </div>
        <div className="text-sm text-gray-600">Celkem: {data.total}</div>
      </div>

      <form className="mt-4 grid grid-cols-1 gap-3 rounded-md border border-gray-200 bg-white p-3 md:grid-cols-6" method="GET" action={ROUTES.CMS.COMMENTS}>
        <input type="hidden" name="sort" value={sortKey} />
        <input type="hidden" name="dir" value={sortDir} />
        <input type="hidden" name="per_page" value={String(data.perPage)} />

        <label className="block md:col-span-2">
          <div className="text-xs font-semibold text-gray-600">Hledání (text)</div>
        ...
        <div className="md:col-span-6 flex flex-wrap items-center gap-2">
          ...
          <Link className="rounded-md border border-gray-300 bg-white px-3 py-1.5 text-sm font-semibold text-gray-900 hover:bg-gray-50" href={ROUTES.CMS.COMMENTS}>
            Reset
          </Link>
        </div>
      </form>
      ...
      <div className="mt-3 overflow-hidden rounded-md border border-gray-200">
        <table className="w-full text-sm">
          ...
          <tr>
            ...
            <th className="px-3 py-2">
              <SortHeaderLink label="Typ cíle" href={sortLink('commentable_type')} active={sortKey === 'commentable_type'} dir={sortDir} />
            </th>

            <th className="px-3 py-2">
              <SortHeaderLink label="Cíl" href={sortLink('commentable_id')} active={sortKey === 'commentable_id'} dir={sortDir} />
            </th>
          ...
          {data.rows.map((r) => (
            <tr key={r.id} className="hover:bg-gray-50">
              <td className="px-3 py-2">
                <Link className="text-blue-700 hover:underline" href={ROUTES.CMS.comment(r.id)}>
                  {r.id}
                </Link>
              </td>

              {staff ? <td className="px-3 py-2">{r.user_label ?? `Uživatel #${r.user_id}`}</td> : null}

              <td className="px-3 py-2">{r.commentable_type.split('\\').pop() ?? r.commentable_type}</td>
              <td className="px-3 py-2">#{r.commentable_id}</td>
                ...
                <td className="px-3 py-2 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <Link
                      ...
                      href={ROUTES.CMS.commentEdit(r.id)}
                    >
                      Upravit
                    </Link>
                    <DeleteCommentInlineButton commentId={r.id} />
                  </div>
                </td>
              ) : null}
            </tr>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-sm">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Link
            ...
            href={buildHref(ROUTES.CMS.COMMENTS, { ...commonParams, sort: sortKey, dir: sortDir, page: String(Math.max(1, data.page - 1)) })}
          >
            Předchozí
          </Link>
          ...
          <Link
            ...
            href={buildHref(ROUTES.CMS.COMMENTS, { ...commonParams, sort: sortKey, dir: sortDir, page: String(Math.min(data.lastPage, data.page + 1)) })}
          >
            Další
          </Link>
        </div>
